...
public class BatteriesManagementController extends Controller {
    ...
    public void showShip(BatteriesManagement data) {
        ...
        for (Node node : spaceshipGrid.getChildren()) {
            ...
            PowerCenter matchingPowerCenter = null;
            for (PowerCenter powerCenter : powerCenters) {
                if (powerCenter.getId() == id) {
                    matchingPowerCenter = powerCenter;
                    break;
                }
            }
            ...
            if (matchingPowerCenter != null) {
                ...
                boolean[] batteries = matchingPowerCenter.getBatteries();
                int batteryIndex = 0;
                    ...
                    if (child instanceof HBox hbox) {
                        for (Node iconNode : hbox.getChildren()) {
                            if (iconNode instanceof ImageView iconView) {
                                if (batteryIndex < batteries.length && batteries[batteryIndex]) {
                                    iconView.setImage(getImageForBattery());
